...
Page({
  ...
  initializePayment() {
    ...
    const apply = (settings) => {
      if (generation !== (this._cartGeneration || 0) || this._paymentTouched || this.data.receivedTouched) return;
  ...
  addToCart(g) {
    ...
    if (hasSkus || (Array.isArray(g.colors) && g.colors.length)) {
      ...
      wx.navigateTo({
        ...
        events: {
          pickSku: (it) => this.mergeIntoCart(it)
        }
      });
    } else {
      // 兼容旧商品
  ...
  mergeIntoCart(it) {
    ...
    if (idx >= 0) {
      if (cart[idx].qty + it.qty > it.stock) {
  ...
  onPriceBlur(e) {
    ...
    const it = cart[idx];
    if (!it) return;
    it.price = util.round2(Math.max(0, util.toNum(e.detail.value, 0)));
    this.setData({ cart: cart });
    this.recompute();
  },
  ...
  recompute() {
    ...
    const needDefault = received < 0 || !this.data.receivedTouched;
    let receivedNum;
    if (needDefault) {
  ...
  onPctBlur(e) {
    let v = Math.min(100, Math.max(0, util.toNum(e.detail.value, 100)));
  ...
  doSubmit() {
    ...
    const valid = cart.filter((it) => it.qty >= 1);
    if (!valid.length) {
