...
Page({
  ...
  async loadDoc(id) {
    try {
      ...
      if (!g) { wx.showToast({ title: '商品不存在', icon: 'none' }); return; }
  ...
  async doSave() {
    ...
    if (f.colors.length === 0 && f.sizes.length === 0) {
      wx.showModal({
        ...
        confirmText: '继续',
        success: (res) => {
          if (res.confirm) this.doSaveAfterCheck(f);
        }
      });
      return;
    }
  ...
  doDelete() {
    const f = this.data.form;
    const total = Math.max(util.skuTotalStock(f.skus), util.skuTotalStock(this._originalSkus));
    if (total > 0) {
      wx.showModal({
        title: '无法删除',
        content: '该商品还有库存 ' + total + ' ' + f.unit + '。请先通过销售/退货清空库存，或改为「停售」。',
        showCancel: false
      });
      return;
    }
    wx.showModal({
      title: '删除商品',
      content: '确定删除「' + f.name + '」？历史订单与流水仍会保留记录。',
      confirmColor: '#e64340',
      success: (res) => {
        if (!res.confirm) return;
        util.removeDocById('goods', this.data.id).then(() => {
