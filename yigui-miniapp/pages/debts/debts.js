    ...
    try {
      ...
      try {
        ...
        customerAgg = {
          ...
          list: (cd.list || []).map((c) => Object.assign({}, c, { totalDebtText: util.fmtMoney(c.totalDebt) }))
        };
      } catch (e) { console.warn('客户聚合失败', e.message); }
      ...
      this.setData({ loading: false, errText: (e && e.message) || '查询失败' });
    }
  },
