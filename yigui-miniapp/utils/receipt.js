...
function drawHeader(ctx, y, order, settings) {
  ...
  ctx.fillText(name, W_CONTENT / 2 + M, y);
  y += 46;
  ...
  if (dt) ctx.fillText(utilFmtDateTime(dt), M + W_CONTENT, y);
  y += 28;
...
function drawItems(ctx, y, order) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    ctx.fillStyle = C_GRAY;
    ctx.textAlign = 'left';
    ctx.fillText((price ? price.toFixed(2) : '0.00') + ' x ' + qty, M, y);
    y += 26;
  }
  dashedLine(ctx, M, y, M + W_CONTENT);
  y += 8;
...
function drawTotals(ctx, y, order) {
  y = drawMoneyRow(ctx, y, '合计', fmt(order.subtotal));
  const pct = Number(order.discountPct) || 100;
  if (pct < 100) {
  ...
  y = drawMoneyRow(ctx, y, '应收', fmt(order.amountDue), { bold: true, big: true });
  y = drawMoneyRow(ctx, y, '实收', fmt(order.received));
  const change = Number(order.change) || 0;
  const debt = Number(order.debt) || 0;
  if (change > 0) y = drawMoneyRow(ctx, y, '找零', fmt(change), { color: C_GRAY });
  if (debt > 0) y = drawMoneyRow(ctx, y, '欠款(记账)', fmt(debt), { color: C_RED, bold: true });
  return y;
}
...
function drawReceipt(ctx, W, order, settings, opt) {
  void opt; // 兼容 dryRun 预测量流程：两遍绘制（第二遍在画布 resize 后），测量与尺寸无关
