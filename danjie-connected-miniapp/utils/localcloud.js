...
// ═══════════════════════════════════════════════════════════════════════════
// 本地云开发模拟层（LocalCloud）
// 用途：当 wx.cloud 不可用（测试号 AppID 无法开通云开发）时，注入本对象，
//       让页面里所有 wx.cloud.database() / wx.cloud.callFunction() 调用
//       全部落到设备本地 storage。这样无需云开发也能在模拟器/真机里真实可用。
//
// 不修改任何页面代码 —— 页面仍照常写 wx.cloud.xxx，本层在 app.js 里替换掉 wx.cloud。
//
// 实现范围（与 9 个云函数逻辑逐一对齐）：
...
async function fnStats(event) {
  ...
  const goodsMap = {};
  salesAll.forEach((o) => (o.lines || []).forEach((l) => {
    ...
    if (gid) {
      ...
      goodsMap[gid].amount = round2(goodsMap[gid].amount + (Number(l.amount) || 0));
    }
  }));
  ...
  salesAll.forEach((o) => (o.lines || []).forEach((l) => {
    if (l.color !== undefined && l.color !== null && l.color !== '') {
      ...
      colorMap[l.color].amount = round2(colorMap[l.color].amount + (Number(l.amount) || 0));
    }
    if (l.size !== undefined && l.size !== null && l.size !== '') {
      if (!sizeMap[l.size]) sizeMap[l.size] = { qty: 0, amount: 0 };
      sizeMap[l.size].qty += (Number(l.qty) || 0);
      sizeMap[l.size].amount = round2(sizeMap[l.size].amount + (Number(l.amount) || 0));
    }
  }));
...
async function fnStatsDebtCustomers(OPENID) {
  ...
  sales.forEach((o) => {
    ...
    if (!map[key]) map[key] = { customerId: o.customerId || '', customerName: o.customerName || '未指定', customerPhone: o.customerPhone || '', totalDebt: 0, orderCount: 0 };
