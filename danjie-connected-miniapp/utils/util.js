...
function ensureOpenid() {
  ...
  if (!wx.cloud) {
    return Promise.reject(new Error('当前环境不支持云开发，请检查基础库版本'));
  }
  if (!loginPromise) {
    loginPromise = wx.cloud.callFunction({ name: 'login' }).then((res) => {
      const r = res.result || {};
...
async function callFn(name, data) {
  try {
    const res = await wx.cloud.callFunction({ name: name, data: data || {} });
    const r = res.result || {};
...
function fetchSettings(force) {
  ...
  settingsPromise = ensureOpenid().then(async () => {
...
async function importAllData(jsonText, mode) {
  ...
  const app = getApp();
  if (app && app.globalData) app.globalData.settings = null;
  settingsPromise = null;
  return result;
}
