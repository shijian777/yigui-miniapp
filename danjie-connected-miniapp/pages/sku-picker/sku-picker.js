...
Page({
  ...
  stepQty(e) {
    ...
    let q = this.data.qty + delta;
    if (q < 1) q = 1;
    if (this.data.selectedSku && q > this.data.selectedSku.stock) {
  ...
  onQtyBlur(e) {
    let q = Math.floor(util.toNum(e.detail.value, 1));
    if (q < 1) q = 1;
    if (this.data.selectedSku && q > this.data.selectedSku.stock) {
      wx.showToast({ title: '库存仅剩 ' + this.data.selectedSku.stock, icon: 'none' });
      q = this.data.selectedSku.stock;
  ...
  /* ---------- 进价（仅进货模式） ---------- */
  onCostBlur(e) {
    const v = util.round2(Math.max(0, util.toNum(e.detail.value, 0)));
  ...
  confirm() {
    ...
    if (this.data.mode === 'purchase') {
      ...
      const evt = {
        ...
        size: sku.size,
        qty: this.data.qty,
        unitCost: sku.costPrice > 0 ? sku.costPrice : this.data.defaultCost,
        stock: sku.stock
      };
      ...
      const evt = {
        ...
        size: sku.size,
        qty: this.data.qty,
        price: sku.price > 0 ? sku.price : this.data.defaultPrice,
